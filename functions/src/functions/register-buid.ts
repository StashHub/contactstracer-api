...
async function registerUserIfNotExists(
...
async function registerBuid(
    ...
    throw new functions.https.HttpsError("deadline-exceeded", "Failed to generate BUID");
}
...
async function createTuids(
    ...
    throw new functions.https.HttpsError("internal", "Failed to generate TUID");
}
...
export const registerBuidCallable = buildCloudFunction().https.onCall(async (data, context) => {
    ...
    if (!hasVerifiedPhone && !hasUnverifiedPhone) {
        throw new functions.https.HttpsError("failed-precondition", "Missing phone number");
    } else if (hasVerifiedPhone && hasUnverifiedPhone) {
        throw new functions.https.HttpsError("invalid-argument", "Bad parameters");
    }
    ...
    if (!(await hasSpaceforBuids(users, fuid, MAX_BUIDS_PER_USER))) {
        throw new functions.https.HttpsError("resource-exhausted", "There are already too many registered devices in your account");
    }
